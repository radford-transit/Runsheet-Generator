...
public class Route implements Comparable<Route> {
  ...
  private int getRouteID(String name) throws Exception {
    if (name.equals("Mega Bus Connect")) return 40;
    else if ((!(Character.isDigit(name.charAt(0)) && Character.isDigit(name.charAt(1))))
        && !name.equals("Mega Bus Connect")) throw new Exception("Not a route shift.");
    return Integer.parseInt(name.substring(0, 2));
  }

  /**
   * Returns the name of the route, given a string containing the route ID and the name of the route
   *
