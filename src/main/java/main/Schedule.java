...
public class Schedule {
  ...
  public ArrayList<RouteDrivingShift> routeDrivingShifts = new ArrayList<RouteDrivingShift>();
  ...
  public ArrayList<TrainingShift> trainingShifts = new ArrayList<TrainingShift>();

  /** The set of shift changes */
  public ArrayList<ShiftChange> shiftChanges = new ArrayList<ShiftChange>();
  ...
  public Schedule(Date date, String[] includedPositions) throws Exception {
    ...
    for (int i = 1; i < CSVReader.csvRecords.size(); i++) {
      ...
      if (new Date(csvRecord.get(CSVReader.Header.DATE)).equals(this.date)
          && Arrays.asList(includedPositions)
              .contains(csvRecord.get(CSVReader.Header.POSITION_NAME))) {
        ...
        this.add(
            CSVReader.recordDescribesRouteDrivingShift(csvRecord)
                ? new RouteDrivingShift(
                    ...
                    new TimePeriod(
                        new TimePoint(csvRecord.get(CSVReader.Header.START_TIME)),
                        new TimePoint(csvRecord.get(CSVReader.Header.END_TIME))),
                    new Employee(
   ...
   * @throws Exception
   */
  public Schedule(Date date, String[] includedPositions, ShiftChange firstShiftChange)
      throws Exception {
  ...
  private void add(Shift shift) throws Exception {
    if (shift instanceof RouteDrivingShift) this.add((RouteDrivingShift) shift);
    else if (shift instanceof TrainingShift) this.add((TrainingShift) shift);
  ...
  private void add(RouteDrivingShift shift) throws Exception {
    if (this.routeDrivingShifts.size() == 0) this.routeDrivingShifts.add(shift);
      ...
      else this.routeDrivingShifts.add(shift);
    else
  ...
  private void add(NonRouteDrivingShift shift) {
    if (this.nonRouteDrivingShifts.size() == 0) this.nonRouteDrivingShifts.add(shift);
      ...
      else this.nonRouteDrivingShifts.add(shift);
    else
  ...
  private void add(TrainingShift shift) {
    if (this.trainingShifts.size() == 0) this.trainingShifts.add(shift);
    else if (this.trainingShifts.size() == 1)
      if (shift.compareTo(this.trainingShifts.get(0)) == -1) this.trainingShifts.add(0, shift);
      else this.trainingShifts.add(shift);
    else
  ...
  public RouteDrivingShift lastShiftOnRoute(Route route) throws Exception {
    for (int i = this.routeDrivingShifts.size() - 1; i > 0; i--) {
      if (route.equals(this.routeDrivingShifts.get(i).route)) return this.routeDrivingShifts.get(i);
    }
    throw new Exception();
  }
